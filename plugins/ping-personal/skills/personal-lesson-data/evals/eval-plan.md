# Eval Plan: personal-lesson-data
...
personal-lesson-data captures data-boundary lessons. Invoked by the personal-lesson master
router (or directly), it runs a hard-rule gate, a duplicate check, then APPENDS the lesson
to `~/.claude/lessons/personal-lesson-data.md`. Browse mode reads from there plus the seed
...
| --- | --- | --- | --- |
| E01 | SKILL.md frontmatter | name=personal-lesson-data + description present | name/desc corruption (F01) |
| E02 | SKILL.md body | references `~/.claude/lessons/personal-lesson-data.md` | append-target drift (F02) |
...
- `skill_frontmatter`: name=personal-lesson-data + a description.
- `append_target_matches_name`: body references `~/.claude/lessons/personal-lesson-data.md`.
...
- None. Structural integrity is deterministic. (The *quality* of a captured lesson is the
  human author's responsibility, not something this skill generates -- nothing to judge.)

## Ship Gate

- `eval.ps1` exits 0. F02 (append-target drift) is the highest-severity failure -- it would
  silently route captured lessons to the wrong file with no error.
