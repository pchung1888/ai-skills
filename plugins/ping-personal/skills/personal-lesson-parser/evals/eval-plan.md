# Eval Plan: personal-lesson-parser
...
personal-lesson-parser captures parser / ingestion lessons (PDF extraction, CSV ingest,
HTML scraping, staging pipelines, idempotency, confidence tracking, tax-year handling).
Invoked by the personal-lesson master router (or directly), it runs a hard-rule gate, a
duplicate check, then APPENDS to `~/.claude/lessons/personal-lesson-parser.md`. Browse mode
reads from there plus the seed lessons embedded in this SKILL.md.
...
| --- | --- | --- | --- |
| E01 | SKILL.md frontmatter | name=personal-lesson-parser + description present | name/desc corruption (F01) |
| E02 | SKILL.md body | references `~/.claude/lessons/personal-lesson-parser.md` | append-target drift (F02) |
...
- `skill_frontmatter`: name=personal-lesson-parser + a description.
- `append_target_matches_name`: body references `~/.claude/lessons/personal-lesson-parser.md`.
