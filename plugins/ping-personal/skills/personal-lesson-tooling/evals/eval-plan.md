# Eval Plan: personal-lesson-tooling
...
personal-lesson-tooling captures tooling / process lessons (git, GitHub CLI, worktrees,
hooks, settings.json, agent dispatch, PowerShell, Windows shell gotchas, subagent cost).
It is also the zero-hit fallback domain for the master router. Invoked by the router (or
directly), it runs a hard-rule gate, a duplicate check, then APPENDS to
`~/.claude/lessons/personal-lesson-tooling.md`. Browse mode reads from there plus the seed
...
| --- | --- | --- | --- |
| F01 | frontmatter name/description corrupted | skill stops triggering / the router's fallback breaks | code |
...
| --- | --- | --- | --- |
| E01 | SKILL.md frontmatter | name=personal-lesson-tooling + description present | name/desc corruption (F01) |
| E02 | SKILL.md body | references `~/.claude/lessons/personal-lesson-tooling.md` | append-target drift (F02) |
...
- `skill_frontmatter`: name=personal-lesson-tooling + a description.
- `append_target_matches_name`: body references `~/.claude/lessons/personal-lesson-tooling.md`.
